...
const SectionTitle: React.FC<SectionTitleProps> = ({ title, sortOption, onSortChange, isDarkMode }) => {
  ...
  return (
    <div className="px-6 py-6 flex justify-between items-center relative">
      <div className="flex items-center gap-2">
        <div className={`w-1.5 h-6 rounded-full shadow-lg ${
          isDarkMode ? 'bg-yellow-400 shadow-yellow-400/30' : 'bg-[#0B1D33] shadow-[#0B1D33]/20'
        }`}></div>
      ...
      <div className="relative">
        <button 
          ...
          className={`flex items-center gap-2 text-[10px] font-black tracking-widest transition-colors uppercase ${
        ...
        {isDropdownOpen && (
          <>
            ...
            <div className={`absolute right-0 mt-2 w-48 border rounded-2xl shadow-2xl z-50 overflow-hidden py-1 transition-all ${
              ...
              {options.map((opt) => (
                <button
                  ...
                  className={`w-full px-4 py-3 flex items-center gap-3 text-[10px] font-black tracking-widest uppercase transition-colors hover:bg-black/5 ${
                    sortOption === opt.id 
